...
from data_needs_reporter.report.classify import classify_threads, pack_thread, save_predictions
from data_needs_reporter.report.entities import extract_entities
...
def test_llm_repair_attempt_then_fail(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setenv("OPENAI_API_KEY", "test-key")
...
def test_generate_comms_respects_cap(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    cfg.comms.nlq = 3
    provider = MockProvider(response={'content': {'body': 'Issue detected', 'bucket': 'data_quality', 'subject': 'Alert', 'text': 'Where are the anomalies?', 'parsed_intent': 'data_gap', 'tokens': 2000}})
    client = RepairingLLMClient(
        provider=provider,
    ...
    summary = generate_comms(cfg, 'neobank', tmp_path / 'comms', client, guard, seed=111)
...
def test_generate_comms_hits_requested_volume(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    pl = pytest.importorskip('polars')
    monkeypatch.setenv('OPENAI_API_KEY', 'test-key')
    cfg = load_config(DEFAULT_CONFIG_PATH, None, env={}, cli_overrides={})
    ...
    cfg.comms.nlq = 10
    provider = MockProvider(response={'content': {'body': 'Resolved ingestion issue', 'bucket': 'analytics', 'subject': 'Daily sync', 'text': 'Show ingestion backlog', 'parsed_intent': 'data_health', 'tokens': 60}})
    client = RepairingLLMClient(
        provider=provider,
        model=cfg.classification.model,
        api_key_env=cfg.classification.env_key_var,
    ...
    summary = generate_comms(cfg, 'marketplace', tmp_path / 'comms2', client, guard, seed=222)
    ...
    assert abs(slack.height - cfg.comms.slack_threads) <= max(1, int(0.05 * cfg.comms.slack_threads))
    assert abs(email.height - cfg.comms.email_threads) <= max(1, int(0.05 * cfg.comms.email_threads))
...
def test_prefilter_demotes_low_signal():
    ...
    results = prefilter_messages(records, users, threshold=0.35, fallback_per_day_channel=0)
...
def test_pack_thread_respects_caps():
    ...
    packed = pack_thread(messages, exec_user_ids={101, 102}, max_messages=20, max_tokens=200)
...
def test_classify_threads_and_save_predictions(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    pl = pytest.importorskip('polars')
    monkeypatch.setenv('OPENAI_API_KEY', 'test-key')
    provider = MockProvider(response={'content': {'theme': 'data_quality', 'relevance': 0.87}})
    client = RepairingLLMClient(
        provider=provider,
        model='mock-model',
        api_key_env='OPENAI_API_KEY',
...
def test_classify_threads_parse_error_limit(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    monkeypatch.setenv('OPENAI_API_KEY', 'test-key')
